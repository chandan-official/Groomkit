...
export default function Otpregister() {
  ...
  const handleVerify = async () => {
    ...
    try {
      const res = await axios.post('http://192.168.139.205:3002/auth/verify', { phone, otp: otpCode });
      if (res.data.success) {
  ...
  const handleResend = async () => {
    try {
      const res = await axios.post('http://192.168.139.205:3002/auth/send', { phone });
      if (res.data.success) {
